...
In this post I'm going to set up the enviroment to test the queries and mutations, and test the `langs` query. It's the same query created in the previous post [Basic example Laravel + GraphQL](/blog/basic-example-laravel-graphql).
...
And now, just run `vendor/bin/phpunit` to test the query.